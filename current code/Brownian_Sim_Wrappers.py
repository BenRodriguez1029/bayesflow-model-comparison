...
class GBMVolWrapper:
    ...
    def GBM_sim(self, v1, v2, v3, x0=np.array([100, 100, 100]), time=100 / 365, time_step=1 / 365):
        ...
        for _ in range(0, int(time / time_step) - 1):
            ...
            correction = 0.5 * \
                np.sum([sigma[:, j] ** 2 for j in range(0, 3)], axis=0)
            drift = drift_coef - correction
            timescaled_drift = drift * time_step
            random_shocks = sigma @ self.RNG.normal(
                scale=np.sqrt(time_step), size=3)
    ...
    def __call__(self, x0=np.array([100, 100, 100])):
        drawn_prior = self.prior()
        motion = self.GBM_sim(**drawn_prior, x0=x0)
...
class GBMCorrWrapper:
    ...
    def GBM_sim(self, rho12, rho13, rho23, x0=np.array([100, 100, 100]), time=100 / 365, time_step=1 / 365):
        ...
        for _ in range(int(time / time_step) - 1):
            correction = 0.5 * \
                np.sum([sigma[:, j] ** 2 for j in range(3)], axis=0)
            drift = b - correction
...
class ABMVolWrapper:
    ...
    def __call__(self, x0=np.array([100, 100, 100])):
        drawn_prior = self.prior()
        motion = self.ABM_sim(**drawn_prior, x0=x0)
